...
(async () => {
    ...
    // ✅ Move exportLines inside the async block so it can access 'files'
...
// Add at end of your conversion script
const exportLines = files.map(file => {
    const name = path.basename(file, '.svg').replace(/(^\w|-\w)/g, s => s.replace('-', '').toUpperCase());
    return `export { default as ${name} } from './${name}';`;
  });
  fs.writeFileSync(path.join(outputDir, 'index.ts'), exportLines.join('\n'));
  console.log('✅ Generated src/icons/index.ts');